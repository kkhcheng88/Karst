...
### ★ 關鍵發現:「收費」深度分析 **70-80% 免費可讀**(用戶唔訂閱,所以呢點決定成件事)

實測兩篇,規律一致:

| 期 | 免費段有咩 | 牆落喺邊 |
|---|---|---|
| **#55 混合鍵合** | **BESI、ASMPT、Ziptronix** + 硬規格:銅鍵合距離 **6μm(TSMC 現行)→ 4.5μm(2029 目標)→ 3μm(原型)**、traditional micro-bump 30-40μm、表面粗糙度 **<1nm**、對準精度 **100nm** | 第 3.5 節後(**70-75% 免費**) |
| **#53 NAND 控制器** | **慧榮 SIMO、群聯(Phison)、Mext** | 4.1 節前(**75-80% 免費**) |

**牆嘅位置本身係重點**:佢斬喺「**邊個贏**」嘅投資結論之前 —— 即**佢收費嗰部分正正係 Karst 自己會做嘅嘢
(判斷),佢免費派嗰部分正正係 Karst 要嘅嘢(名 + 機制)**。商業模式同需求啱啱好互補。

**數據質素逐篇差好遠,唔可以當篇篇都硬**:#55 規格紮實可驗;#53 大部分定性冇出處(「1GB」「幾千字」
「200-300 層」)。

### ⚠️ 實際限制:佢最好嘅名大部分**唔喺美股**
BESI(阿姆斯特丹)、ASMPT(香港)、群聯(台灣)、台達電(台灣)—— Karst 係美股框架,買唔到。
**得慧榮 SIMO 有美股 ADR。**
**但呢個反而可能更有用**:如果混合鍵合嘅價值真係俾非美設備商賺走,**呢個係 advanced-packaging
theme(現用 AMKR/KLIC/ONTO 等美股名)嘅負面證據 → magnitude 要打折**。**red-team 素材,唔係買入清單。**

### 判決(2026-07-17):**加入 —— 免費層 + 收費文預覽,零成本,做「發現 + red-team 證據」用**
- **唔訂閱**(用戶已定:太貴,且 70-80% 已免費)
- **唔做買入來源**(名多數非美股)
- **實測(而家零成本,做得過)**:攞 #55 **免費段**行一次 INGEST → 餵到 advanced-packaging 嘅
  magnitude / kill_metric 或出到 WEAKEN(非美廠賺走價值)= 證實有用;全 NEUTRAL = 好睇嘅嘢

⚠️ **本評估限制**:1 篇免費全文 + 2 篇收費文免費段 + 20 預覽。「45% 有出處」係**免費週報**嘅數。
**唔由一篇推及全刊**(同日已犯三次同類錯,見 §3 尾)。
