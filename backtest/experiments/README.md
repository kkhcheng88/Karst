...
| exp_chain_spotcheck.py | 真期權鏈 spot-check:LEAP constant-m 定價 model(IV_1y=m×VIX/100)vs live SPY/QQQ chain IV(BSM 逐 strike 算 delta)+ skew 檢查 + 短 call 0.30Δ/21DTE spread + IV rank | 2026-07-09_options_chain_spotcheck.md | active 🟡(只驗今日平靜市 base level,唔驗 crash-time term structure;0.50Δ 今日 m≈1.0-1.2 貼近 grid,但 0.70/0.80Δ skew 令真 IV implied m 達 1.3-2.0,遠超 grid 上限 1.15——deep-ITM 低估比已知 crash-vega 誤差更嚴重;短call spread 1.03% 貼近 0.5%/邊假設) |
| exp_magnifier_case_library.py | Phase-3 magnifier 模型逆向工程樣本庫 Leg B(價格篩:sp500_px+px_defeatbeta 兩宇宙,任何 24-42 月窗 ≥5x,起點 2010-2024) | 2026-07-09_magnifier_case_library.md | **raw 樣本庫,非訊號回測**(Leg A 人手重建 9 個超級週期贏家+輸家對照 + Leg B 883 隻價格篩命中;跨週期收斂:護城河獨立於商品價格程度 + 週期頂前有冇槓桿式產能/併購 = 最強 ex-ante 贏輸分野;SMCI/WOLF 兩個「贏家變輸家」case 直接驗證咗 repo thesis wiki 已寫低嘅 kill_condition) |
| exp_filing_signal.py | WS4 早期偵測增量:10-K/10-Q(CCJ 用 40-F/6-K)全文 constraint-language 掃描(同 exp_constraint_language.py 同一詞表,MD&A+Risk Factors 段,EDGAR urllib 直抓)vs 同期 transcript,6 case(MU/LLY/VST/CCJ/AXTI 真.供需超級週期 + RGTI negative control),三比對:時機/增量/Lazy-Prices 逐份文字改動 | 2026-07-09_filing_vs_transcript_signal.md | **可行性+增量探測,非回測**(negative:filing 劣過 transcript,唔係 redundant——4 個可判斷 case 全部 filing 唔早於 transcript(MU/LLY/CCJ 全窗未曾轉正、VST 只喺 Risk 短 stub 季度同步跟到);增量方向反轉:transcript 熱詞(lead times/allocation/sold out)filing 幾乎唔用,filing 反而帶入通用鬆動樣板(oversupply/discounting/pricing pressure)拖低淨分數,LLY 尤其傷(pricing pressure 喺 pharma = PBM壓價非供需);Lazy-Prices 逐份新增語言雜訊大、冇單調早期訊號;儲存代價中位數 ~2.4x transcript;RGTI negative control 兩邊近零但 filing 側「近零」靠同一批通用樣板詞拖平,非乾淨訊噪分離;唔建議 filing 全文入 Phase-3 常規語料) |
| exp_buyback_capital_allocation.py | Mauboussin/Chancellor 資本配置行為訊號複現:buyback_yield(trailing-4Q 回購/市值)+ net_issuance(trailing-4Q 淨發行/市值)cross-sectional IC + quintile spread(63/126/252d,defeatbeta quarterly_cash_flow + market_capitalization point-in-time,SPDR 11 板塊 top-10 廣宇宙),分板塊拆解判斷是否集中喺週期/materials/energy | 2026-07-11_buyback_capital_allocation_signal.md | active 🟡(訊號方向啱、pooled rank-IC 126d/252d 兩訊號皆 t>2.9,但 quintile-spread lens 弱(t<1.9 全部);唔集中喺 Phase-3 target 嘅 materials/energy——反而 Financials 最強、Discretionary 次強,Energy/Materials 近零;數據只覆蓋 2022Q2-2026Q1 單一 regime,樣本期短;建議收編做人手 4-KPI checklist 項目,唔升做 machine lint 硬閘) |
| exp_ballast_parking.py | 全風險組合「偽現金」測試:200SMA 閘出咗 SPY 之後,泊喺 XLP/XLU/XLV 等權防守籃(W3) vs 泊喺現金^IRX(W2),4 warehouse(W1 SPY B&H/W2/W3/W4 籃子B&H),FULL+兩半(2012split)+三壓力窗(2008/2020/2022),HK稅 r_net,10bps/邊,冇參數搜索(等權指定) | 2026-07-12_ballast_parking_ab.md | active(negative:W3-W2 bear-window 每 episode 平均 -1.51%——防守籃子跌市期間仍跟跌,唔係好嘅現金替代;96 episode 入面錯配型態清晰——非壓力窗 whipsaw 期籃子跑贏現金,但真崩盤期(2008/2020)籃子照跌;W4 牛市租金全期-1.03pp/yr 但 regime-dependent:H1 1999-2011 反而 +1.37pp,H2 2012+ 達 -2.66pp,近期 regime 租金遠高於全期平均) |
| exp_bt2_aa_structure.py | BT-2 全風險組合 Karst-AA **結構** vs core-v2 vs SPY B&H:T sleeve=QQQ beta proxy(零 thesis-alpha,純結構測試),ballast(XLP/XLU/XLV 等權)+ 月度 delta 總帳 sized 0.50Δ LEAP overlay(SPY+QQQ per-leg 200SMA 閘)+ Parked(strict)/Cash(pragmatic);strict/pragmatic 雙變體 × base/damp 雙 IV × band{1.00/1.15/1.30};2001+ 8-way 共同窗;複用 exp_core_topup 引擎+exp_ballast_parking loader,cross-foot assert(307k) | 2026-07-12_bt2_aa_vs_core.md | active ✅(結構結論:AA MaxDD ~-44% 遠淺過 core-v2 -54.5%/SPY -55.6%,worst-12m 亦贏;代價係 raw α 細(strict base +7.5pp t+5.0 vs core-v2 +12.4pp)——差距 ≈ 零-alpha T sleeve 留低嘅空間;**strict > pragmatic**(+7.5 vs +5.4pp base,MaxDD 相若)——ballast 研究嘅 14pp strict tax 喺全結構唔出現(trio 兩變體都係主 sleeve);Bonferroni×48 base 全 6 格 PASS,damp 只 strict@115/@130 PASS(pragmatic damp FAIL,同 core-v2 damp 邊緣一致);DSR≥0.996;strict underfund 事件=1 已披露;core-v2 Step-0 覆現 base+12.38/damp+6.44pp 全在接受帶) |
| exp_bt7_ballast_def.py | BT-7 ballast(防守 sleeve)**定義**比較:B1 XLP/XLU/XLV(現行)vs B2(+USMV)vs B3(+VIG)vs B4(USMV獨)vs B5(DIA,正式候選)vs B6(Mag7等權月度reb,明文負面對照),判官=downside capture+三壓力窗+126d QQQ相關+HK淨CAGR/稅漏(職責達成度排名,唔鬥靚回報),三 league 共同窗(USMV/VIG/Mag7-META窗,B1 每 league 重新計做錨)+ beta-租金咬合表加熊市 delta 地台(0.60β+0.30)欄,cross-foot assert(269k) | 2026-07-13_bt7_ballast_definition.md | active ✅(B1 三個 league 總分皆最低=最佳,維持現行 trio;B5 DIA 意外地 downside capture 差(92-98%,近乎 1:1 跟 SPY 跌)+ QQQ 相關全部候選中最高,「低科技含量」假設證偽;B6 教學位驗證通過——熊市 delta 地台 109.1%(>100%),LEAP 全閘出都冇得減磅,量化「offense 資產做 ballast」代價) |
| exp_bt_b_washout_expr.py | BT-B washout(市場廣度洗盤)**表達層**對比:同一訊號(a50 p10 washout episode,複用 exp_breadth_reversion_verify episodes())、同一時序(T收市確認/T+1入場,21d+42d),SPY 現貨 vs 0.80Δ 6個月 SPY call(base m0.85+damp0.4 並列),同本金 vs 同曝險兩口徑;episode 名單溯源(gap-sweep 20-300td 證明無單一 gap 重現原檔 8-12,改用 14 個日曆年 crisis-window 分組 + 9 個嚴重度過門檻 = headline),FINE(37)/CRISIS-ALL(14)/CRISIS-SEVERE(9)三顆粒度並列 | 2026-07-13_bt_b_washout_expression.md | active(表達層對比,非訊號驗證;headline n=9 同本金21d期權贏8/9但同曝險21d只贏2/9——優勢主要嚟自槓桿/資本效率,非期權喺同風險下本質更優;n太細唔用t-stat) |
