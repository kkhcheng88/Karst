...
6. **EDGAR 數據線不應押後。** 本票原本的問法是「若 OSAP 夠用,EDGAR 應否押後」,但同日用戶已落 D-048,明確更正財報數據線為「來源已驗證、欠接線」(財報主幹 = SEC EDGAR,見 D-041),並把因子庫由「用來發現策略」降為「用來支援已有策略前提」。加上本票自己查到的三個結構性缺陷(20 個月滯後、重述數據、REIT 缺席),結論是:**OSAP 不是 EDGAR 的替代品,兩者用途不同,不存在誰押後誰的取捨。** OSAP 的位置見第二節。
...
- **這一輪的定位是校準,不是發現。** 同日 D-048 已定:全市場跑只保留一個用途——拿有公開答案的訊號跑一次,證明本平台算得對;它是驗機器,不是搵策略。OSAP 正好是理想的校準對象,因為它連「正確答案」(官方公布的多空組合月報酬序列)都一併免費提供。用它來**發現**基本面選股策略,與 D-048 相衝,不要做。
- 實測窗口的終點是 **2024-12**,不是今日。任何「這條策略現在會揀邊幾隻」的問題,OSAP 答不到。
- 結果要標明「建基於重述後的會計數字」,即實際可實現的績效大機率低於測出來的數字。這不是可以事後補救的細節,是解讀結果的前提。
- REIT 要明文排除出實測宇宙(或者另行處理),不要讓 42 隻缺值股票靜靜地變成偏差來源。

**問:SEC EDGAR 數據線應否押後?**

**不應押後。** 本票開票時的問法假設了「OSAP 若夠用就可以頂替 EDGAR」,查證之後這個假設不成立,而且同日用戶裁決亦已否定這個方向:

- **用戶已更正過這件事(D-048,2026-08-30)**:財報與逐字稿的數據線由「未有來源」更正為「**來源已驗證、欠接線**」——財報主幹是 SEC EDGAR(D-041),DefeatBeta 提供逐字稿與損益表。即是說,EDGAR 不是一個「待評估的選項」,是已經定案的主幹;欠的是把它接進 Karst 的管線,不是找替代來源。
- **OSAP 結構上做不到生產所需的三件事**:實時性(20 個月滯後,且無更新承諾)、真正的知情時點口徑(重述問題無法從這一側修正)、完整宇宙(REIT 與 ETF 缺席)。這三項正好是 EDGAR 的核心價值,不是「遲些再算」的優化項。
- **所以兩者不是取捨關係,是分工關係**。OSAP 在這套系統裡有兩個位置,兩個都不與 EDGAR 重疊:
  1. **校準基準**——用它公布的多空組合報酬,驗證我們自己新建的分組多空計算件(KARST-101)算得對。這是 D-048 給全市場跑保留的唯一用途。
  2. **收窄宇宙之後的因子支援**——按 D-048,先有可證偽的策略前提收窄宇宙,再用因子在收窄後的宇宙裡找時機與做確認。那 117 條財報類因子可以直接充當這一步的現成材料,省下自建因子定義的工。
- **要留意 D-048 第 4 點的禁令**:不可以先揀定一批股票,再去因子庫問哪幾條與之最夾。用 OSAP 的 117 條時,條件必須寫在前面。
