# -*- coding: utf-8 -*-
"""KARST-204:替判斷隊砌「遮蔽版」取證包。

一包一宗事件。包內只有衝擊日之前拿得到的東西:
  - 事件敘事、衝擊起訖日;
  - 籃子成員(代號、名、CIK、SIC)、衝擊窗相對跌幅(步六動作二要用);
  - 衝擊日之前最後一份 10-K / 10-Q 的申報日、期末、accession、本地全文路徑或 EDGAR 網址;
  - 衝擊日之前最後一期財務面板(收入、現金流、負債等),連期末與申報日。

**明文遮蔽**:T_3m/6m/12m、N_3m/6m/12m 六欄一律不入包——那是本票要考的答案。
遮蔽由這支腳本機械保證,不靠判斷隊自律。
"""
import csv
import json
...
OUT = f"{BASE}/checklist_test/packets"
...
def main():
    ...
    summary = []
    for eid, members in sorted(by_ev.items()):
        ev = events[eid]
