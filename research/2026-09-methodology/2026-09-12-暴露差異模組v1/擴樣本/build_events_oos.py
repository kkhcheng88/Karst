# -*- coding: utf-8 -*-
"""KARST-220 擴樣本(E21–E27):籃子、衝擊窗、T/N 兩錨與十二個月超額。

**算法照 KARST-199 `build_baskets.py` / KARST-218 `build_new_events.py` 一字不改的口徑**
(直接 import 199 的 `basket_core`:同一套 `load_spy` / `load_prices` / `build_wide` /
`resolve_tickers` / `PanelFeatures` 與同一條 `excess()` 進場規矩),只做兩件事:
  1. 輸入換成 `new_events_spec_oos.json`、輸出落到 `擴樣本/out/`;
  2. **市值欄不用 `PanelFeatures.mcap_usd`**(已知缺陷:它是覆蓋頁股數 × 價格庫 `adj_close`,
     對高息股系統性少算、對拆股公司嚴重少算;KARST-218 舉手第 1 件、KARST-219 修復中)。
     改用 **未還原收市價 `close` × 最近申報流通股數**,欄名 `mcap_oos`。

輸出:
  out/event_list_oos.csv      事件清單(日期、敘事、來源、籃子大小、籃子相對跌幅、T 低點)
  out/basket_members_oos.csv  逐宗逐家(解析狀態、衝擊跌幅、T/N 兩錨 3/6/12 個月超額、事前特徵)
  out/unresolved_oos.csv      解析不到的點名代號(倖存者偏差正本)
  out/dispersion_oos.csv      籃內分散度(新聞點名與同 SIC 全體,兩錨)
"""
...
SPEC = HERE / "new_events_spec_oos.json"
...
def main() -> None:
    ...
    # 未還原收市價寬表(市值修復用,不用 adj_close)
    raw = (px.pivot_table(index="date", columns="entity_id", values="close", aggfunc="last")
             .reindex(cal))
    ...
    for ev in events:
        ...
        for kind, mem_list in (("新聞點名", members), ("同SIC全體", sic_members)):
            for e in mem_list:
                ...
                px_close = float(s.iloc[i0])          # adj_close,只供面板原樣輸出
                dv = dollar_vol.get(e)
                rec["dollar_vol_60d"] = float(dv.iloc[max(0, i0 - 59):i0 + 1].median()) if dv is not None else np.nan
                feats = pf.at(e, t0, px_close)
                rec.update(feats)
                # --- 市值修復:mcap_oos = 未還原收市價 close × 最近申報流通股數
                #     股數由面板 mcap_usd ÷ 當日 adj_close 反推(同一份覆蓋頁股數),
                #     再乘未還原收市價;兩者缺一即留空,不猜。
                adj0 = float(s.iloc[i0])
                raw0 = float(raw[e].iloc[i0]) if e in raw.columns and np.isfinite(raw[e].iloc[i0]) else np.nan
                m0 = feats.get("mcap_usd", np.nan)
                if np.isfinite(m0) and np.isfinite(adj0) and adj0 > 0 and np.isfinite(raw0):
                    rec["mcap_oos"] = float(m0 / adj0 * raw0)
                else:
                    rec["mcap_oos"] = np.nan
                rec["close_raw_i0"] = raw0
                rec["adj_close_i0"] = adj0
                rev = feats.get("ttm_revenue", np.nan)
                eq = feats.get("equity", np.nan)
                ni = feats.get("ttm_net_income", np.nan)
                mc = rec["mcap_oos"]
                rec["f_ps_oos"] = (mc / rev) if (np.isfinite(mc) and np.isfinite(rev) and rev > 0) else np.nan
                rec["f_pb_oos"] = (mc / eq) if (np.isfinite(mc) and np.isfinite(eq) and eq > 0) else np.nan
                mem_rows.append(rec)

        ev_rows.append(dict(
            event_id=eid_, name=ev["name"], event_type=ev.get("event_type", ""),
            ...
            sources=" | ".join(f"{s['outlet']} {s['date']} {s['url']} [{s.get('verified','')}]"
                               for s in ev["sources"]),
            notes=ev.get("notes", "")))
        log(f"      {eid_} {ev['name']} [{ev.get('event_type','')}]: 點名 {len(conf)} → 可用 {len(members)}"
            f",籃子相對跌 {basket_drop:.1%},低點 {t_trough.date()},新聞結束 {cal[i_news_end].date()}")

    evdf = pd.DataFrame(ev_rows)
    memdf = pd.DataFrame(mem_rows)
    evdf.to_csv(OUT / "event_list_oos.csv", index=False, encoding="utf-8-sig")
    memdf.to_csv(OUT / "basket_members_oos.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(unres_rows).to_csv(OUT / "unresolved_oos.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(rows).to_csv(OUT / "dispersion_oos.csv", index=False, encoding="utf-8-sig")
    log("完成。")
    log(evdf[["event_id", "event_type", "shock_start", "trough_date", "basket_rel_drawdown",
