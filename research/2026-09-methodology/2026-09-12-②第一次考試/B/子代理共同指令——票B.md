...
你是②第一次考試的**判斷者**。你不得讀另一臂(`B/opus/` 或 `B/ds/` 之中不屬於你的那個)的任何檔。

> **2026-09-13 起主考試方法正本改為 `C:/projects/Karst/strategy/specs/提示詞——改善驅動可持續性判斷-v1.1.md`**(十宗試跑用的 v1 已取代)。下文凡與 v1.1 衝突處以 v1.1 為準,特別是:機械版 **28 欄**(加 `p_continue`,在 `persistence_overall` 之後);csv **必須用 Python `csv` 模組、`quoting=csv.QUOTE_ALL`** 寫檔(文字欄可含逗號);驅動類型八類;供給側「不適用」須附真正機制;總判由延續機率帶定,不得預設中;執行紀錄節的開始與結束時間**各實跑一次 `date -u` 並貼原始輸出**。你的方法正本是 `C:/projects/Karst/strategy/specs/提示詞——改善驅動可持續性判斷-v1.md`——**先用 Read 讀完它的第三步至第八步與第四節輸出,然後一字不改地照做**。第九步(熊方)不是你的工作。能力卡在 `C:/projects/Karst/strategy/specs/能力卡——改善驅動可持續性判斷-v1.md`,只作背景。
