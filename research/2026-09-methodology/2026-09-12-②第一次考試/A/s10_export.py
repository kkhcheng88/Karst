# -*- coding: utf-8 -*-
"""KARST-222 票 A:輸出三份交付檔 —— population.csv、entry_pool.csv、thresholds.md。
...
population.csv 欄位:
  ...
  價格 —— 反應報酬、相對 SPY、相對同 SIC2 同日中位、60 日中位成交額
...
ORDER = ["accessionNumber", "cik", "name", "ticker", "form", "sic", "sic2", "bucket",
         ...
         "dvol_med_60", "gate_volume", "first_px_date", "listed_months",
...
REASON = [("excl_no_price", "無日線"), ("excl_volume", "60 日中位成交額不足 300 萬"),
...
def main() -> None:
    ...
    L = []
    add = L.append
    add("# ②第一次考試 · 票 A 逐年門檻與事件數(thresholds.md)")
    add("")
    ...
    add("")
    add("注:第 95/80 版的數目只作靈敏度參考;執行口徑 v1 的入口池定義用第 90 百分位。")
