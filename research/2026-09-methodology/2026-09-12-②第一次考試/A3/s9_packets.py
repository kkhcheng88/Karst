# -*- coding: utf-8 -*-
"""KARST-226 票 A″(v1.2)第十步:逐事件建 T1 遮蔽取證包 `packets/<event_id>.json`。

與 `A2/s9_packets.py` 之別:
  ①讀 A3 的鎖定清單(`A3/picks_before_results.md` / `cache/picks.json`);
  ②第 4 項財務數列:訊號季那一行另標 `source = "EX-99.1 稿內文字"`,並列出稿內抽到的
    收入數(`rev_signal_text`),XBRL 值並列供對照(執行口徑 v1.2 第 4 項);
  ③第 1 項補 `release_timing`(盤前/盤中/盤後)與 `t0_source`(8-K 受理時間或稿頭日期)、
     `dateline`(執行口徑 v1.2 第 2 項、補二);
  ④第 1 項不再有 `cluster_id`(v1.2 無此欄);provenance 指向 v1.2 修訂頁。
包體七項、遮蔽規則、原文快取(`A/edgar_cache/`,已抓者不重抓)一字不改。
...
10-K/10-Q 全文不入包,只留本地路徑與 EDGAR 網址。
...
from s5_fetch_text import _get           # 同一個克制抓取器(3 請求/秒、短重試)
...
WORKERS = 8                  # 見執行紀錄「跑不通、規格不清」:請求速率仍為 3/秒,
                             # 多連線只是填滿等 SEC 冷物件的死時間
...
def get(url: str, timeout: int = 25) -> bytes:
    """沿用 s5 的抓取器(短重試、快放棄;SEC 冷物件約 10 秒,長退避會拖垮整支)。"""
...
def main() -> None:
    ...
    for p, m, pf in jobs:
        ...
        pack = {"event_id": eid, "schema": "②第一次考試取證包 v1", "ticker": m["ticker"],
                ...
                "provenance": {"rule": "執行口徑——②第一次考試-v1.2(修訂頁)第 1–10 項"
                                       "(門檻、時點、資料三路修訂);原口徑 v1 第二節;"
        ...
        if b.get("ok") and q_end:
            ...
            if i is not None:
                ...
                for e in ends[max(0, i - 7): i + 1]:
                    rev_filed = filed_map.get(e, (None, ""))[1]
                    # 逐列**不寫申報日期**(可能是 T1 之後的日子,見驗收條件第五條:包內不得
                    # 出現 T1 之後的日期);只寫布林值,說得出「這列的 XBRL 首報晚於 T1」。
                    series.append({
                        "period_end": e,
                        ...
                        "ocf": b["ocf_q"].get(e),
                        "revenue_xbrl_first_filed_after_T1": bool(
                            rev_filed and rev_filed > t1),
                        "revenue_year_end_derived": bool(
                            e not in b.get("rev_q_direct", ()))})
        # 訊號季那一行:收入來源改標 EX-99.1 稿內文字(v1.2 第 4 項);XBRL 值並列供對照
        for r_ in series:
        ...
        pack["4_財務數列"] = {
            ...
            "source_by_row": "見 quarters[].revenue_source",
        ...
        pack["masking_check"] = {
            ...
            "known_leak_note":
                ...
                "`4_財務數列.n_rows_first_filed_after_T1` —— 那些非收入項目在 T1 那天尚未"
                "進入公開申報(多數屬該季 10-Q 才披露的項目),屬已知輕微越界,不影響收入"
                "方向的判斷(收入已改用稿內文字並另列 XBRL 值對照)。",
