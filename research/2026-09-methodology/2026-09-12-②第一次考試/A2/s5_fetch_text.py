# -*- coding: utf-8 -*-
"""KARST-225 票 A′(v1.1)第五步:抓觸發那份 8-K 的 EX-99.1 全文(判指引上調正則用)。

與 `A/s5_fetch_text.py` 之別:①強勢反應子集改用 A2 的宇宙(v1.1 門檻);
②原文快取沿用 `A/edgar_cache/`(**不另起一個**,已抓者一律不重抓,新增者寫回同一個);
③單線程、3 請求/秒(票面硬規矩)。已抓成功者由 `__EX991.txt.gz` 存在與 fetch_log 兩重判斷。
...
CACHE = HERE / "cache"
DOCS = HERE.parent / "A" / "edgar_cache"    # 沿用 v1 的原文快取(票面許可)
UA = {"User-Agent": "Karst research kaho@example.com"}
N_WORKERS = 1               # 票面硬規矩:補抓單線程
RATE_PER_SEC = 3.0          # 票面硬規矩:3 請求/秒
