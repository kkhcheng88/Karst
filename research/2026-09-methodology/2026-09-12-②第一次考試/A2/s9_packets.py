# -*- coding: utf-8 -*-
"""KARST-225 票 A′(v1.1)第九步:逐事件建 T1 遮蔽取證包 `packets/<event_id>.json`。

與 `A/s9_packets.py` 之別:讀 A2 的宇宙與鎖定清單(`A2/picks_before_results.md`);
原文快取沿用 `A/edgar_cache/`(已抓者不重抓,新抓者寫回同一個快取)。包體七項、遮蔽規則
一字不改。A2 的 84 包全部由本檔新建(不從 `A/packets/` 複製),執行紀錄另列與 A 同
accessionNumber 者的逐節比對。
