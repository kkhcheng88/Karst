...
半數無效的會在格上打一個星。孤峰打三角、平原打點——圖例逐個寫明。
...
from .verdict import INVALID, LONELY_PEAK, PLATEAU, SweepJudgement
    ...
    *,
    ...
    marks = {
        (row[x_axis], row[y_axis]): (
            int(row["cells"]), int(row["invalid"]), int(row["plateaus"]), int(row["lonely_peaks"])
        )
        for _, row in frame.iterrows()
    }
    ...
    axes.set_xticks(range(len(xs)), [_tick(v) for v in xs])
    axes.set_yticks(range(len(ys)), [_tick(v) for v in ys])
    axes.set_xlabel(x_axis)
    axes.set_ylabel(y_axis)

    for yi, y_value in enumerate(ys):
        for xi, x_value in enumerate(xs):
            cells, invalid, plateaus, peaks = marks.get((x_value, y_value), (0, 0, 0, 0))
            if cells == 0:
                continue
            if invalid == cells:
                axes.add_patch(
    ...
    legend = (
        "▲ 孤峰   · 平原   * 部分無效   斜線格 = 整格無效(成交太少)   "
        "淨灰格 = 這個組合根本不存在(例如兩個權重加起來已經超過一)"
        if has_cjk
        else (
            "^ lonely peak   . plateau   * partly invalid   "
...
def write_report(
    ...
    *,
    ...
    lines.append(f"- `{verdict_csv.name}`:逐格裁決、鄰域平均、落差")
    lines.append("")
    lines.append(
...
def _tick(value: Any) -> str:
    ...
    if isinstance(value, (int, float)):
        number = float(value)
        if 0.0 <= number <= 1.0:
            return f"{number:.0%}" if abs(number * 100 - round(number * 100)) < 1e-6 else f"{number:g}"
        return f"{number:g}"
    return str(value)
