...
原本只問表,於是取不到數。三件事要證:
  ...
  3. **表那條路照舊**:值住表的因子讀回來與 ``latest_known_values`` 逐格相同;
     一條因子兩邊都有值時,兩邊都看得見。
...
from karst.engine.factorvalues import FactorValueSource
...
def test_latest_known_value_matches_the_parquet_and_stops_at_the_knowledge_gate(
    gateway, values, snapshot_id, entities
):
    source = FactorValueSource(gateway.store)
    day = DAYS[2]

    taken = source.latest_known_values(BATCH_FACTOR, day, entity_ids=entities)
    ...
    later = source.latest_known_values(BATCH_FACTOR, DAYS[LATE_KNOWN_ON], entity_ids=entities)
...
def test_table_values_read_exactly_as_before(gateway, snapshot_id, entities):
    ...
    for day in DAYS:
        taken = source.latest_known_values(TABLE_FACTOR, day, entity_ids=entities)
        before = gateway.store.latest_known_values(TABLE_FACTOR, day, entity_ids=entities)
        assert list(taken.columns) == list(before.columns)
        assert taken.to_dict("records") == before.to_dict("records")
...
def test_table_and_batch_are_read_together(gateway, factor_root, snapshot_id, entities):
    ...
    taken = FactorValueSource(gateway.store).latest_known_values(
        BOTH_FACTOR, DAYS[2], entity_ids=entities
    )
