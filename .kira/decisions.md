...
- 影響:地圖②格、能力卡 v1 證據與權限紀錄檔、報告第八節。**2026-09-14 補記**:用戶裁刪考試目錄內不入 git 的申報原文與中間快取(原話「I think all can remove」),回歸集自此 = git 內 1,087 個檔(取證包、名單、卡、行、評分)加磁碟上的 `population.csv`(母體表,不入 git,須另行備份);修正前九包的備份與原文快取已不存在,原文可按 accession 由 EDGAR 重取。
...
## D-181 獨立投研與策略設計(2026-09-14):中期基本面預期差、研究不讀持倉、評級／計劃／模型配置分開;用戶授權更新 repo 並開始規劃設計
- 類型:決策
- 狀態:有效
- 日期:2026-09-14

- 出處:用戶與 GPT 的 2026-09-14 對話,由 GPT 依用戶授權直接寫入並提交(commit 20fa988);「用戶要求」四項是 GPT 的轉述,用戶原話未逐句照錄;「GPT 設計」九項是 GPT 以投資管理者角色提出的設計裁定。主 agent 2026-09-15 補記:本條「優先於 D-166／D-179」一句改變了項目主軸(四類注由強制分類降為可選描述),這一點待用戶親口確認一次(見 ② 對帳單 §十二 第 25 條)。

- 用戶要求:
  1. 聚焦本日討論的最佳策略與投資管理者設計,不要評論其個別損失或個人心理經驗。
  2. AI 不看用戶持倉,自行研究、評級、給目標價並回顧自己的決策。
  3. 明確要求「can update into the Repo? And start doing the planning or design?」,本輪依此寫入設計及計劃並提交;不是還在等待開始的授權。
  4. 同輪補充「should 基本面 to have Instrisic Value calculated as fair mid-target price?」;依此明列由基本面推導的每股公允價值,並解釋估值日內在價值與有期限的價格目標分開。

- GPT 設計(不冒充用戶逐項指定):
  1. 主策略為中期基本面預期差:產業理解為基礎、事件更新、掃描補盲點。舊四類注保留可選描述與歷史研究,不要求分類後才分析,不預設單向遷移或四類等配。
  2. 研究、模型組合、評估分邊界。正式研究以新上下文與來源／工具允許清單運行,不讀真實或模型帳本、成本與盈虧;開發對話已見資料不稱盲分析。從現行投資委託移除私人持倉／帳戶清單,不改寫 git 歷史。
  3. 六層與反方圍繞同一論點因果鏈;強模型讀原文,程式算數,不用票數或總分代替綜合裁決。日週月、pivot、200日SMA、通道與突破回踩保留,SMC選用。
  4. L3 推導經營／每股現金流,L4 算估值日公允價值基準與情境範圍、現價折讓及敏感度;中期期限目標另有經營／估值／催化劑橋接。六個月評級目標日為設計預設,不是用戶新增持有期偏好;用戶一月至一年保持。
  5. 來源原文／版本、研究發布與持久營運狀態分開。不可變發布保留原目標與依據,SQLite索引可重建但任務／游標／帳本需持久備份。價格日更新TA、估值的現價要求／折讓及計劃,新業績／產業證據按訂閱與依賴傳播。
  6. 先交通用獨立股票頁(P1/KARST-240),再觀察名單與增量(P2)、模型組合與評估(P3)、擴大掃描與日報(P4)。WeKnora有界試用並行,以任務品質與總成本決定,不作證據正本或股票頁前提。
  7. 研究頁不需私人淨值;模型組合另定資本與風險委託後開始紙上執行,只允許發布後模擬成交。原1%仍為示例,不復活舊槓桿／停手預設。
  8. 本輪審過九份 insights 的結論與取捨,未重新核完原著頁碼。其類別轉移、資金離場鐘與注碼數字不自動成為全域規則。
  9. D-180通用程式要求保留;參數化 fixture 可有代號與可信預期值,不得因此複製每股腳本。工作票按通用能力開,不用第二股再開取證腳本票。

- 與舊文的關係:本條對現行工作台的策略／隔離／交付次序優先於 D-166／D-179 及其文件中衝突段落,不重寫原用戶原話與凍結研究。D-178的84宗保留及品質回歸不變。本輪完成策略與設計文件,未完成 runtime、股票頁、模型績效或部署。

- 正本:strategy/投資決策模型.md、strategy/投資委託.md(v1.2)、strategy/specs/六層分析紀律-v1.md(v1.2)、strategy/specs/相位與階段清單-v1.md(v1.1)、strategy/specs/獨立投研工作台設計與交付計劃-v1.md。Epic與D09–D12身份不變,進度同步地圖、HANDOFF、KARST-240與交付登記。

## D-182 用戶裁(2026-09-15):四類注的地位——不是強制入口,也不只是可選描述,而是可啟用的情境分析模組,由共同投資決策框架統領;估值方法跟隨生意的經濟結構,情境只決定哪些假設要特別審查;AI 不准自由挑一個能支持結論的估值法;每次估值必答四問(方法為何適合、依賴哪三個假設、另一視角得什麼、與上次相比為何變)
- 類型:決策
- 狀態:有效
- 日期:2026-09-15

- 出處:用戶 2026-09-15 貼來的立場全文(以 GPT 起草的文字作答,與 D-169、② 對帳單 §十二 第 5 條同一做法),回應主 agent「四類注降為可選描述要用戶親口確認」一問。原話首句:「我認為方向正確,但『降為可選描述』說得太輕。更準確的定位應該是:四類注保留為可啟用的情境分析模組,整個系統由共同的投資決策框架統領。」末句:「如果由我主導 Karst,我會採用『共同框架＋按經濟特徵選估值＋按情境加問』。目前取消四類注作強制入口的決定我會保留;但它們應成為有作用的分析模組,而不只是卡片上的標籤。」

- 背景:D-181(GPT 起草)把四類注寫成「保留作描述與材料索引」;主 agent 指出主軸改變要由用戶講。用戶答覆把它定為情境模組:每個模組決定該情境要特別攻哪些問題(潛在錯殺攻永久損害與價值陷阱;改善趨勢攻持續性與現價已反映多少;商業化敘事攻誰付錢、何時形成現金流、此前要多少融資);啟用模組是待證假設,叫「錯殺」之後研究要攻「市場為何沒錯」。估值三件分清:計算是否成立(可檢查)、經營假設是否可信(要來源與後續業績驗證)、市場何時反映(估值公式不保證)。主 agent 立場(D-110):完全同意,而且這比 D-166 的強制分類與 D-181 的可選描述都好——它把兩張既有能力卡(①暴露差異、②改善持續性)與檢查清單 v3 直接變成兩個模組的攻擊問題清單,③④則誠實地只有問題沒有方法;估值方法按經濟結構選、四問必答,把「AI 自由估值」的最大漏洞堵住。一句補充:啟用模組的理由本身要有證據(例如「潛在錯殺」要有可核的被迫賣家或行業衝擊事件),否則模組會變成給結論穿的衣服。

- 決策:
  1. 四類注 = 可啟用的情境分析模組,由 `strategy/投資決策模型.md` 的共同框架統領;不強制歸類,不等額配置,不預設遷移;啟用理由要寫明並有證據
  2. 三個模組的攻擊問題清單:潛在錯殺(①能力卡 + 檢查清單 v3)、改善趨勢(②能力卡)、商業化敘事(③④,只有問題清單,無已驗證方法);寫入六層紀律 L3「按情境模組加問」
  3. 估值方法跟隨經濟結構四型(成熟現金流 / 週期正常化 / 商業化前虧損 / 金融企業);情境只決定哪些假設要特別審查;由趨勢變潛在錯殺通常是換輸入與情境,不是換公式
  4. 每次估值必答四問,輸出欄位 `method_rationale`、`top3_assumptions`、`alternative_view`、`change_attribution`;不同視角的結果不准平均;不准為保住原評級悄悄換方法(寫入六層紀律 L4)
  5. D-181「優先於 D-166」一句由本條收窄:D-166 的四類注定義與用戶原話不變,改變的是它們在系統中的角色

- **用戶原話(原文照錄)**

  > 我認為方向正確,但「降為可選描述」說得太輕。更準確的定位應該是:四類注保留為可啟用的情境分析模組,整個系統由共同的投資決策框架統領。[…] 估值方法應主要跟隨生意的經濟結構,情境則決定哪些假設要特別審查。[…] 我支持給 AI 一個通用框架,但不支持讓它自由挑一個能支持結論的估值法。[…] 如果由我主導 Karst,我會採用「共同框架＋按經濟特徵選估值＋按情境加問」。目前取消四類注作強制入口的決定我會保留;但它們應成為有作用的分析模組,而不只是卡片上的標籤。

- 影響:`strategy/投資決策模型.md` §一 兩段改寫;六層紀律 L3 加問改按情境模組、L4 加四問與四欄位;地圖 §零 表頭一句;實作(KARST-240)仍等用戶講「開始」。

- **2026-09-15 補記(用戶同日第二段立場,同一做法)**:第三個模組改稱**「敘事與預期差」**,商業化只是其中一個問題分支,其他分支為替代威脅、供需轉變、轉型與資本配置;敘事不只屬於③④,每隻股票都要答共同敘事問題(市場相信什麼、我們同意或不同意哪一部分、差異怎樣影響未來每股價值、現價值得付多少),模組在敘事成為主要爭議時加深,至少追查五件事(敘事內容、證據狀態、經濟傳導、市場預期、驗證與反證);三個模組不是互斥箱子。潛在錯殺的啟用條件放寬:被迫賣家與行業衝擊是入口但不是唯一入口,公司特定事件亦可;啟用要可核查的事件與假說,「不需要先證明市場錯了才准研究市場是否錯了」。用戶原話:「我會用『敘事與預期差模組』作名稱,並把『商業化兌現』放在其中,按需要啟用。商業化太窄,不能涵蓋我們真正想分析的敘事。[…] 我不會把原本四類注重新包裝成三個互斥箱子。」主 agent:全部接受;我原句「必須有被迫賣家或行業衝擊」收窄過頭,改為「可核查的事件與假說」。這五件事正是 D-166 framework §四 論點卡八格裡的敘事證據、共識度量、定價位置、推翻條件、時限——用戶原本的論點卡規格在這個模組裡活下來,沒有丟。

## D-183 用戶裁(2026-09-15):工作台建置分工——GPT 負責技術架構、四份契約(證據登記、取證包、研究輸出、發布包)、通用核心、測試與可閱讀股票頁,以分支加合併請求交付;本地(主 agent)負責公開市場樣本包、真實接口整合、實際模型運行、`.kira/`、覆核與合併;三步交付:步一離線核心與股票頁(KARST-241)、步二接真實來源與模型跑首批兩隻(KARST-240)、步三更新一次並加 SQLite 與差異路由(KARST-242)
- 類型:決策
- 狀態:有效
- 日期:2026-09-15

- 出處:用戶 2026-09-15 貼來的兩段立場(以 GPT 起草的文字作答):第一段提出分工(「我的角色可以是技術架構與核心程式的負責者」、KARST-240 拆三步、「已保存的證據包和即時抓取的資料走同一套分析核心」、情境模組是問題不是新 agent);主 agent 回以四個條件;第二段用戶答「我同意這四個條件,尤其是第四點」並補三個實作要求。

- 背景:主 agent 四個條件——契約先行且結構定義檔為機器格式唯一正本;程式碼走分支加合併請求、本地跑測試才合併、`.kira/` 治理檔由本地維護;樣本只給公開市場資料;第一步驗收是頁面不是資料庫。用戶三個實作要求——取證包必須允許補查(補入登記版本,發布時固定實際使用的證據集合,「否則會把日常研究做回封閉考試」);樣本包保留真實缺漏與口徑(空值、缺逐字稿、不同財務期間、幣別、單位;一家公司開始接線,之後另一種生意驗通用性);頁面完成與完整投研完成分開驗收(離線樣本驗計算、引用、版本、呈現;模型讀原文、補證、結論是否有用要本地連線跑)。第一步的檔案與發布包仍要保留來源身份、版本、時間、引用及依賴,第三步沿用不重建格式。主 agent 立場:同意;這個分工把主 agent 放回整合與顧問位;風險在契約漂移與 GPT 的沙盒看不到真實資料,由「契約定義檔加樣本包」兩件對沖。

- 決策:
  1. GPT:模組邊界、四份契約的版本化結構定義檔與測試、通用讀取／計算／發布／頁面程式、可重現指令;交付經分支與合併請求,說明列交付路徑、完成條件與剩餘缺口,並分開交代「頁面完成到哪」與「完整投研未做哪些」
  2. 本地:公開市場樣本包(一隻首批股票,保留真實缺漏;帳戶與持倉類回傳一律不入)、真實接口整合與實測、實際模型運行與運行紀錄、`.kira/` 維護、本機跑測試後決定合併
  3. 契約:結構定義檔是機器格式唯一正本,Markdown 只解釋含義,測試只查行為;取證包允許補查;步一用檔案登記與發布包,SQLite 與自動差異偵測延後到步三,但版本、時間、引用與依賴欄位由步一起保留
  4. 三步票:KARST-241 步一 → KARST-240 步二(依賴 241)→ KARST-242 步三(依賴 240);觀察名單、掃描、模型組合、WeKnora 沿設計計劃接上
  5. 第一個共同里程碑:同一份契約、同一套程式,在兩邊都能產生來源可追查的股票頁
  6. 開工時點:樣本包抓取是本地第一次跑取數,等用戶講「開始」(D-180 第 4 條)

- **用戶原話(原文照錄)**

  > 我同意這四個條件,尤其是第四點:第一步以可閱讀、可追查的股票頁驗收,SQLite 和自動差異偵測可以延後。[…] 取證包必須允許補查。[…] 樣本包要保留真實的缺漏與口徑。[…] 頁面完成與完整投研完成分開驗收。[…] 這個分工可以採用。第一個共同里程碑就是:同一份契約、同一套程式,在兩邊都能產生來源可追查的股票頁。

- 影響:KARST-241、242 開票;KARST-240 定為步二並依賴 241;`karst/tests/fixtures/README.md` 定樣本規矩;地圖 D12 格與下一步隊列。
