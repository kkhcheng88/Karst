...
### agent:karst-130-agent · 2026-09-01 02:03
收檔。由今日起平台每日儲低 SPY 連十一隻 SPDR 板塊 ETF 的在外股數,落 `vintage/fund-shares/` 追加式版本存檔;單位數的每日變化就是申贖流,這條線一年之後就不用再靠富途反推(2020 前凍結分母,D-094)。

日課由一句變兩句,先設 `PYTHONUTF8=1` 在倉根順序跑 `python -m karst.gateway multiples capture` 與 `python -m karst.gateway shares capture`;HANDOFF 第三節第 6 項已更新。另有 `shares list --symbol XLK` 看逐次讀數(兩列之差即淨申贖)、`shares backfill` 由已落檔原始頁面補。

回填做到了:第一步查已存的 SSGA 原始頁面,發現本來就有現成欄位——`vintage/sector-multiples/raw/` 三次原始頁面每張都有「Fund Net Asset Value」一節,載 NAV / Shares Outstanding / Assets Under Management 連自報截數日,所以不用另抓發行商頁,直接擴解析回填十二列。但要講清楚:那三次截數日全部是 2026-08-28,回填得回的是一個時點不是三個,真正的逐日序列由今日起才開始累積。

首日證據:readings.csv 2026-08-31(UTC)十二列十二隻齊,抓取時間與截數日兩欄齊,原始頁面先落檔後解析並由 .gitignore 擋走不入 git。同日重跑冪等在真存檔上驗過——回填十二列之後即時真抓一次,十二隻全報「同日不變」、新寫零列,跨命令都不會重覆寫。

驗證:新測試十七條全過零 skipped(含真連網實抓),全套 429 passed / 7 skipped(七條 skip 全屬本票以外),生產庫雜湊 b168e9f4…75fa8 不變(命令攔在 Gateway.open 之前,有測試專守)。

兩件要記住的事。一,不把新欄加落板塊倍數那份 CSV 而另開一份存檔,是主 agent 定案(不是用戶裁決):追加式存檔的規矩是舊列一字不動,加欄就要改寫已寫下去的歷史列;代價是同一張頁面一日抓兩次,已加禮貌節奏逐隻停一秒。二,新記假設 **A-026**——發行商自報的在外股數逐日真更新且不追溯改寫。這正是富途那條線出事的形狀(A-025:2020 前用單一固定股數計,凍結十四年而表面看不出);若為假,自儲線由開檔第一日起就是假的,取代反推線的整個理由消失。查證途徑已內建,累積一兩個月後對「截數日推進的日子」與「單位數變動的日子」即查得出。

詞彙表加「在外股數 / shares outstanding」一條,對齊倉內原本兩個中文叫法(另一個是 `申贖流` 條目寫的「在外單位數」)。

Commit:`ca560de`(存檔件本體)、`47a0559`(假設 A-026)。
