...
- [x] 策略合約與策略執行台落地,因子混合經 Executor.register/run 走完一次正式運行;全部 12 條既有正式運行重跑後運行編號與八項成績指標逐位相同(測試:逐條比對舊 fingerprint)
- [x] 參數文字化與純量驗證各只剩一份正本;參數規格宣告的格數等於掃描格認得的軸數,每格有值域無取值,風控三格在內(測試:缺一格拒收、多一格拒收、值域不合拒收、換倉節奏無預設)
- [x] tests/doubles/engines.py 與 conftest.py 已建,四份假引擎收成兩個;只跑所涉測試檔;備份已做

## 結果

- · 2026-08-30 23:40 karst/executor/contract.py(策略合約、參數規格)與 karst/executor/executor.py(執行台:register/run/rejudge)落地;因子混合首條搬入,舊登記與落痕函數已刪。
- · 2026-08-30 23:40 正式運行實為 13 條(含 1 條測試污染,KARST-093 處理);13/13 運行編號與指紋逐位相同;因子混合那條逐日淨值、持倉、188 筆訂單重跑逐位相同。另 2 條的價格序列在較早前重建數據時已不在倉,與本票無關。
- · 2026-08-30 23:40 tests/conftest.py、tests/doubles/engines.py 新建,四份假引擎收成兩個;14 個所涉測試檔 79 passed;備份 karst.sqlite.2026-08-30-090.bak 已做。
- · 2026-08-30 23:40 舉手「對齊標記是否入庫」由主腦裁決:不入參數集內容,另開 KARST-094 做簽章旁表。
