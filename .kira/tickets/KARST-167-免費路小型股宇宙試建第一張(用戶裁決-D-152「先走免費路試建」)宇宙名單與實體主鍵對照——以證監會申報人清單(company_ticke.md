...
### agent:Claude Opus 5 · 2026-09-03 09:07
\## A-046 推翻:本票 §2.4「按首次申報年」那張表對老公司系統性偏遲

由 KARST-174 留言,票已關,**不倒推階段**,只記錄影響。

本票建的申報索引單一快取 `data/sec/submissions/`,KARST-174 逐份查證發現:EDGAR 的 `CIK<十位>.json` **只載最近約 1,000 份申報**,更早的歷史放在該檔 `filings.files` 欄列出的分頁檔,要另行抓取,而本快取**一個分頁檔都沒有存**。

| 項目 | 數 |
|---|---:|
| 快取檔數 | 8,012 |
| 其中 `filings.files` 非空(申報史被截短) | **2,066(25.8%)** |
| 這批合共聲明的分頁檔 | 2,736 |
| 快取存有的分頁檔 | **0** |

實例:雅培 CIK 0000001800,快取那份只有 2018-06-05 至 2026-08-27 的 1,002 份;1994-02-14 至 2018-06-03 的 2,612 份不在檔內。

\### 對本票結論的影響(逐項)

1. **§2.4「按首次申報年」的家數分佈表不可信,尤其老公司那幾格。** 那 2,066 家讀到的「最早申報日」其實是第 1,000 份那一份,不是真正第一份,所以**首次申報年一律偏遲**。「1995 或之前 851 家」這一格會被系統性低估——真正 1995 年前上市的公司,有一部分被算進了 2006 之後那幾格。
2. **§2.4 的讀法要收回一句。** 本票寫「2016 年之後首次申報的有 1,995 家,佔 37.9%」並據此說這一格對迷因過濾器 R5(上市未滿兩年)很重要。**這個 37.9% 是上限,不是實數**;R5 在那 2,066 家身上會漏判——它們看似新上市,其實是老公司。本票原文已提過借殼上市令這條線偏早,現在再加一個**反方向而且更大**的偏差源。
3. **不受影響的:** 實體表 5,257 家、代號時段表、近似市值、交易所分佈、SIC 構成、135/138 十倍股落點——這些只用 `filings.recent` 的近期資料或今日快照,不靠最早申報日。

\### 補救

`experiments/2026-09-02-silent-revisions/data/submissions/` 現存 1,051 個分頁檔,補得回 2,736 個之中的 971 個(35.5%),令 553 家公司的申報史完整;餘下 1,765 個要打 EDGAR,按本票實測每秒 8 個請求約 4 分鐘。**收編與補抓與否已在 KARST-174 舉手待裁**,裁了之後開善後票,連同 §2.4 重算。

查證腳本 `experiments/2026-09-03-submissions-dedup/check_shard_gap.py`,結果 `out/shard_gap.json`。假設冊已記 A-046(`overturned`)。

順帶一提:本票報告 §四把五個實驗目錄一併數作「副本」,其中三個(lazy-prices、narrative-layers、narrative-layers-v2)其實存的是 10-K 清單衍生物不是申報索引本體,D-134 明文准許,KARST-174 一個都沒有刪。
