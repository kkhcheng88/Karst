...
> 2026-09-14 根基重整(D-177)時由現行簿搬入:V1 藍圖期、V1 建置期與①研究期的一百六十條裁決。**只提供編號供舊票引用追溯,不再約束現行工作**;現行有效的條目全部在 `.kira/decisions.md`。搬入時內容原樣不改(含當時的「狀態:有效」字樣,以本註為準視作退役)。全文另在 tag `pre-reset-2026-09-13`。
...
## D-164 KARST-177 收案:申報索引正本補全——收編 1,051 個歷史分頁與 99 份宇宙外公司主檔(整檔搬移、逐檔雜湊核對零失敗),補抓餘下 1,765 個分頁零失敗(約 9 分鐘、每秒 8 請求),2,066 家被截短公司 100% 補齊;首次申報年分佈重算後逐格與 KARST-167 原表相同——建表程式早已用主檔自帶的邊界日期補足截短部分,D-161 擔心的「老公司被當新上市」在該表上沒有發生;A-046 維持 overturned 並補查證日期與證據(缺失屬實且已修復,但未影響分佈表);完整性另出 completeness.csv 逐家登記,manifest.csv 只寫彙總比率
- 類型：決策
- 狀態：有效
- 日期：2026-09-03

- 出處：KARST-177 交付(票已關;commit a04e6f5;生產庫雜湊 b168e9f45b578cf9 不變)。主 agent 依 D-072 收案。

- 背景：主 agent 立場(D-110):①D-161 那句「首次申報年分佈同樣受影響」是我按 KARST-174 的推論寫的,未經查證,現在證明是錯的;補全本身仍值得做,因為「首次申報日」以外的任何用到完整申報史的規則(8-K 事件時鐘、內部人交易、修訂追蹤)在那 2,066 家都會缺前段。②completeness.csv 另開一份的做法對,不強塞進每個位置一行的表。③本票由 sonnet 做,按票面逐步完成、判斷題一個沒有自作主張,派工模型分類成立。

- 決策：
  1. KARST-177 結論收貨;data/sec/submissions/ 現為完整正本(主檔加全部分頁)
  2. 更正 D-161:首次申報年分佈未受截短影響;「首次申報日當上市日」代理弱的理由改為換 CIK/分拆/ADR 轉本地,不是截短
  3. 三隊已全部交付並入帳,依用戶指示停下等討論
...
## D-172 ①候選池取消「市值 5 億美元以上」閘——市值不再作篩選條件
- 類型：決策
- 狀態：有效
- 日期：2026-09-09

- 出處：用戶 2026-09-09 原話:「市值 5億以上 << I think actually no need this at all」。前文是主 agent 依 KARST-194 加不加閘對照提出「市值閘由閘降為標籤,防走不掉交給每日成交額 300 萬美元線」;用戶的回應比建議更進一步,是整個條件不要。

- 背景：KARST-194(research/2026-09-methodology/2026-09-09-①基準率表/對照——加不加閘.md):在過流動性閘的觸發之中,拆走市值閘平均十二個月超額升 1.42 個百分點、一年內再跌八成比例只升 0.08 個百分點,建表年與驗證年方向一致;歷史十二個月最強 50 筆機會之中 41 筆撞市值閘;假設 A-060(三閘每一道都改善獵場)已推翻,市值閘是其中不成立的一道。市值閘原是 D-169 兩層漏斗初篩的一部分,KARST-184/188 的六十家名單以它為條件之一。

- 決策：
  1. ①候選池篩選條件移除「市值 5 億美元以上」;市值不作閘,亦不作標籤
  2. KARST-197 重出排序時按此口徑重篩;KARST-178 規格書第一節與第八節相應條文改寫,標注源自本決策與 A-060

- **用戶原話（原文照錄）**

  > 市值 5億以上 << I think actually no need this at all

- 影響：主 agent 建議(用戶未明講,待確認):防止買了走不掉的工作交給每日成交額線(基準率表主口徑為 ≥300 萬美元),①池篩選是否已含此線由 KARST-197 核對並寫明。市值閘拆走後池會擴大,1 億至 5 億美元一段的公司進入候選,這一段倖存者缺口最深,基準率表對它的估計偏樂觀,排序表要標明。

## D-173 ①候選池篩選只留「滾動四季經營現金流為正」一道閘加成交額線;負債比率由閘降為標籤;其餘四件判準以主 agent 建議作暫定工作規則,前瞻證據出來再定
- 類型：決策
- 狀態：有效
- 日期：2026-09-10

- 出處：用戶 2026-09-10 原話:「actually I don't know. but I think 只留現金流閘、負債比率降為標籤 make sense. Other than that actually I don't know how to comment.」——現金流閘與負債標籤一項是用戶裁決;其餘四件(回報底線三級、否決者留表另欄、三類機制標籤、預測零支持不得升買)用戶明言不懂評論,由主 agent 依 D-072 以自己建議作暫定規則,標「暫定,待前瞻登記證據」,用戶保留否決。

- 背景：KARST-194 加不加閘對照:只留現金流閘(加成交額 ≥300 萬美元線)十二個月平均超額 +4.7%、剔前 1% 後 +1.3%、一年再跌八成 1.6%,每一項都好過全不設閘(+4.2% / +0.3% / 2.9%);拆負債閘平均回報跌 0.25 個百分點、爆倉率升 0.59 個百分點,兩邊都小;歷史最強 50 筆買得到的機會之中,現行三閘只留 4 筆,只留現金流閘留 25 筆。D-172 已取消市值閘。用戶同時提出真正的問題不在池而在「捕捉」:進場邏輯、評分、R&R 與非結構化資料怎樣用——主 agent 另行講解。

- 決策：
  1. ①候選池篩選閘只剩兩條:滾動四季經營現金流為正;近 60 日中位成交額 ≥300 萬美元(示例值,待參數對齊)。負債對經營現金流比率照算,作標籤與判斷層輸入,不剔人
  2. 暫定工作規則(主 agent 建議,非用戶裁決):(一)回報底線分三級,13–17% 為貼線不下結論;(二)200 日線形態否決者照列排序表另欄標「被否決」,不落注,入前瞻登記;(三)錯價來源改三類機制標籤,倍數重估須寫理由;(四)預登記預測沒有一項支持買入者不得升為「買」
  3. KARST-197 按此口徑重出六十家(池會擴大,重篩以新閘為準);規格書 KARST-178 第一節與第八節改寫時標注源自本決策與 D-172、A-060

- **用戶原話（原文照錄）**

  > actually I don't know. but I think 只留現金流閘、負債比率降為標籤 make sense. Other than that actually I don't know how to comment.

- 影響：池擴大後 1 億至 5 億美元一段與高負債公司進入候選,倖存者缺口最深、基準率偏樂觀,排序表要標;判斷層(卡片)工作量上升,先只對排位前十二家填卡不變。四件暫定規則的成敗以前瞻登記一年後對照為準,不另設到期日(D-171)。
