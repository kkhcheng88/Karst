...
## §2 已驗證嘅查詢範本(2026-07-17 用戶實跑,出品質素高 —— 照抄)

### 範本 A|供給約束掃描(每個日期資料夾跑一次)
```
掃描範圍:投行/大型券商機構研報(高盛/摩根士丹利/摩根大通/瑞銀/花旗/巴克萊/美銀/伯恩斯坦/杰富瑞/德銀/野村)
語料來源:知識庫中 [日期資料夾] 嘅報告
任務:找出被描述為「供給約束/供給緊張」嘅公司或子行業,全市場範圍
強度分級(逐字引句,唔准改寫):
S:sold out / on allocation / 停報停簽 / booked out
A:供不應求 / 供需缺口 / 結構性短缺 / supply deficit
B:交期拉長 12 月+ / lead time extending
C:產能吃緊 / tight supply / bottleneck
D:多輪漲價 / 定價權
E:LTA / 承購協議 / take-or-pay / 預付款
輸出:主表(級別|子行業|原文逐字引句|需求定供給驅動|報告名|日期)+ 未見約束語言嘅子行業 + 引用報告清單 + 局限聲明
```

### 範本 B|theme ticker 掃描(§1 個 85-ticker 清單照貼)

### 用完之後
1. 將 IMA 輸出貼俾 Claude(佢會對照 17 個 theme 出「買全文優先次序」)
2. 買 2-5 份 → 放 `thesis/.raw/ima/pdf/`(檔名加日期 prefix)
3. Claude 行 INGEST(分流 → 紅隊 → theme 更新)
**分工鎖死**:IMA 輸出 = AI 片段整理,只做「搵」;證據 = PDF 全文。
