...
## v2.2 換鏈日期(第二批,2026-09-03,KARST-169)

v2.1 只處理了 MSFT / ORCL,自己列明還有兩處同類前視問題未做。本票補上:
`ai_hpc_hosting` 四家(CORZ / IREN / WULF / CIFR)按換鏈日期改記,HOOD 在 `crypto_exchange`
的入位日核實。

- **正本**:`chain_membership_v2_2.csv`(UTF-8 BOM)、`chain_stories_v2_2.md`、`removed_v2_2.csv`(UTF-8 BOM)
- **腳本**:`fetch_v2_2.py`(補年報入共用快取)、`fetch_evidence_v2_2.py`(抓 8-K / 6-K / 20-F 作佐證)、
  `build_v2_2.py`(改表 + 逐行對帳)
- **v0 / v1 / v2 / v2.1 一字不改**,仍在原處(`build_v2_2.py` 只讀不寫)。

### 轉向日判準(查證之前寫死)

**取首次公開宣佈 AI/HPC 客戶合約的當日。** 只宣佈買 GPU、只宣佈策略、只講「potential HPC
tenants」而未有客戶合約的,不算。這條判準令四個日期全部**精確到日而且事前可得**——
公佈當日 8-K / 6-K 連新聞稿一齊出,不需要等年報倒推(與 v2.1 的 ORCL 那一格不同)。

| 公司 | 轉向日 | 那一日公佈了什麼 |
|---|---|---|
| IREN | 2024-02-08 | 與 poolside AI 的 GPU 雲服務合約正式開始(248 顆 H100) |
| CORZ | 2024-03-06 | 與 CoreWeave 的多年期合約,供應最多 16MW 機位 |
| WULF | 2024-12-23 | 與 Core42(G42 旗下)的長期資料中心租約,逾 70MW |
| CIFR | 2025-09-25 | 子公司與 Fluidstack 的十年期資料中心租約,Google 作後盾 |

### HOOD:日期不改,判詞更正

`crypto_exchange` 的 `valid_from` 維持 2022-01-01(走廊起點之前它已在做加密交易並單獨披露該收入線)。
但核實過程查到 v2 一句判詞是錯的:「2024 年起加密交易佔交易收入主體」不成立——
加密交易占總淨收入 2021/2022/2023/2024/2025 是 23% / 15% / 7% / 21% / 20%,
期權同期是 38% / 36% / 27% / 26% / 25%,**每一年期權都較大**。已改寫。
...
| 不重覆公司數 | 343 | 343 |
| 出隊行數 | 21 | 21 |
| 本票新抓年報 | — | **10 份**(`fetchedBy: KARST-169`) |

逐行對帳(`build_v2_2.py` 內建,不過就報錯):349 行之中 337 行逐格相同、7 行只改了
逐鏈共用的 `purity_note`、5 行在准許改動清單上,另新增 4 行;報錯 0。

### 未做

1. **APLD 仍是 2022-01-01**,同一類前視問題,票面沒有點名,本票沒有動。
2. HOOD 的成員資格未按四條出隊規則重判——只核實日期。
3. `chain_membership_v2_1.csv` 本身有一組重複鍵(`WDC` 在 `memory` 出現兩行),v2 之前已存在,本票不動。
