...
### W3：首份有明確研究問題的雲端正式版本與頁面

2026-09-19 用戶採納 GPT 建議：取消 INTU 的自動優先，先完成 BE 已有的「AI 電力／價值鏈、每股現金流、市場倍數與價格結構」爭議。沿用仍有效的公開證據、更新必要資料並重新檢查關鍵假設；負面、等待或不做均是有效研究結果，不為得到正面評級換股。這是當次選題配置，不把股票身份寫入程式。

依賴 W2 的可用連接；GPT 主研究並接手必要開發。Claude 暫時沒有額度：可保存及發布明示「獨立覆核待完成」的主研究，不冒稱已交叉覆核，W3 的覆核驗收仍未完成。沒有第二模型時不以主研究自查充數，也不把付費 API 覆核默認啟動。沿用既有公開研究與必要對話是本次互動研究的明示背景，不稱盲測；不得取私人持倉、成本或帳戶資料。
...
**首屏驗收：**按投資委託第五節顯示目前判斷、基本面、估值、技術與交易計劃；更新先講差異，機器識別及完整反證保存在 Agent 紀錄。下方包括當日內在價值情境及算式、現價隱含要求、有期限目標的橋接或缺口、支撐阻力／200 日 SMA／日週月解讀、入場／加倉／退出條件與 R&R。未知不能捏造；等待也要說明具體在等什麼，不把可當下補的資料一律推到下一季。
