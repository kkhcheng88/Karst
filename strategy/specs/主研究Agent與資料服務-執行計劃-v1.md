...
- **覆核：**模型呼叫放在獨立 reviewer 模組；只處理足以改變決策的爭議，不把每筆取數串成多 agent 辯論。
...
- **入口：**先驗本地工具，再部署供 GPT 連接；Claude 本地可用同一套工具驗通。ChatGPT 互動研究與未來 API 自動主研究共用核心。
...
| `request_review`、`get_job` | 指定爭議、證據、待覆核研究版本；返回 job ID，後取覆核結果及實際用量。 |
...
- 一名主研究者輸入／輸出格式及提示詞，承接委託與六層必答問題；收件器與現有 renderer 接通。
...
依賴 W1。部署單一服務實例及持久目錄到 Zeabur；用官方 MCP Python SDK 的 Streamable HTTP 接入，鎖定實際驗過的 SDK 版本。部署目錄、啟動命令、環境變數名稱及本地／遠端驗證步驟寫入 README，不提交憑證。

先完成部署配置及本地驗證再處理外部連接；沿用既有帳戶授權。只在確實缺雲端權限、來源憑證或用戶端連接操作時，提出一份具體缺項清單，其餘已授權工作繼續。遠端入口使用客戶端支援的驗證方式，不能為方便把研究寫入或付費覆核 API 裸露在公網。

覆核以 Claude API 的獨立 job 執行；服務明記模型、起訖和可取得用量，設定單次配置的回合／費用上限，避免意外循環。同一 job 去重；已送出模型呼叫但結果不明時標記待核對，不自動再次呼叫造成重複費用。中斷標 interrupted，不假稱成功或不設限自動重跑。

**驗收：**從 GPT 端實際讀一份研究、取一項新資料、執行一個計算及取得一次覆核結果；遠端重啟後仍可讀原研究。研究工具只提供公開資料；券商 token 的 scope 與服務工具清單分開核實。

### W3：第二股完整研究與頁面

依賴 W2；預設 INTU，代號只是配置。GPT 在新的研究上下文中執行，帶清理後委託、六層紀律與工具，沿用本系統已發布的公開研究可明示提供；不帶開發對話、私人持倉或成本。本輪 GPT 對話已含私人經歷，不把這條舊對話直接當獨立研究 worker。

主研究流程：
1. 讀既有認知及資料目錄，列會改變判斷的問題。
2. 讀最新適用申報與逐字稿全文，查財務、產業與價值鏈、共識及行情；自由補查網頁／用戶報告。
3. 建立因果論點與情境，選估值方法，調計算工具。分析相位與日週月位置，形成條件計劃。
4. 把最重要爭議交 Claude 覆核；反方可讀原文並補查，回具體挑戰及依據，不以另一評級作投票。
...
- `agents/research.py`、`agents/prompts/research.md`：單主研究輸入／收件；`agents/review.py`：API 覆核。
