...
> **現行產品入口(D-179)**:先交付能每日使用的股票頁,再逐步擴充。生產角色開工讀 `strategy/specs/六層分析紀律-v1.md`;目前為 v1.1、六角色,TA 核心是關鍵 pivot／支撐阻力／200 日 SMA,日週月分工、通道與突破回踩,SMC 選用。舊的「先研究一宗事件」、「每格已量度／未量度」、A-043 風險預設與封閉考試取證條件不作現行工作台指令。歷史回歸仍使用其凍結協議,不讓生產規格改寫舊考題。
...
10. **程式必須通用,倉內不准一次性腳本**(用戶 2026-09-14 明令,D-180):程式只住 `karst/` 套件,代號與日期由參數傳入,檔名與碼內不出現任何股票代號;探索與除錯碼放倉外 scratchpad,不 commit;派工指令必須帶這條原文,工人交回的股票專屬檔不收貨。細則 `karst/README.md`。
11. **用戶問問題不等於叫開工**(用戶 2026-09-14:「I am asking you on the phases and stage but not asking you to start」):要用戶講明「開始」才派工或開實作票;討論與清單只答問題。

其餘規矩:`HANDOFF.md`(現況與規矩)、`CONTEXT.md`(詞彙表)、`.kira/decisions.md`(已裁決的事)。

2026-09-14 GPT 審查同步:本次為已授權的規格修訂,不代表股票頁、取證器或 WeKnora 已部署;進度以地圖 D12 與下一步隊列為準。
